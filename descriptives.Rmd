...
round(summary(df$netsover2)[3], digits = 0)
round(summary(df$netsover2)[4], digits = 0)
...
dens1 <- ggplot(netsize_l, aes(x=value, color = key)) + 
  ...
  theme(legend.position = "none") +
  labs(x = "Network size estimate", y = "Density") +
  scale_color_manual(values = c("grey", "#56B4E9", "grey", "grey")) +
    geom_vline(xintercept=246, color = "darkgrey", linetype = 2) +
  geom_vline(xintercept=282, color = "darkgrey", linetype = 2) +
  annotate("text", x = 350, y = 0.0008, color = "black", angle = 90, 
           label = "Mean = 282") +
  annotate("text", x = 175, y = 0.0008, color = "black", angle = 90, 
           label = "Median = 246") +
  xlim(c(0, 1500))
# save
