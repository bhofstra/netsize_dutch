...
summary(df$numwoman)
summary(df[df$woman == 1, c("numwoman")])
summary(df[df$woman == 0, c("numwoman")])

summary(df$numman)
summary(df[df$woman == 1, c("numman")])
summary(df[df$woman == 0, c("numman")])

summary(df$samegender)
summary(df[df$woman == 1, c("samegender")])
summary(df[df$woman == 0, c("samegender")])

summary(df$numeduchigh)
summary(df[df$opl2 == 1, c("numeduchigh")])
summary(df[df$opl2 == 0, c("numeduchigh")])

summary(df$numeduclow)
summary(df[df$opl2 == 1, c("numeduclow")])
summary(df[df$opl2 == 0, c("numeduclow")])
...
# Comparing four different estimation scenarios
...
round(summary(df$netsover4)[3], digits = 0)
round(summary(df$netsover4)[4], digits = 0)

dens1 <- ggplot(netsize_l, aes(x=value, color = key)) + 
  geom_density(alpha = .2, size = 0.5) + 
  theme_minimal() + 
  theme(legend.position = "none") +
  labs(x = "Network size estimate", y = "Density") +
  scale_color_manual(values = c("grey", "grey", "grey", "#56B4E9", "grey")) +
    geom_vline(xintercept=446, color = "darkgrey", linetype = 2) +
  geom_vline(xintercept=518, color = "darkgrey", linetype = 2) +
  ...
  xlim(c(0, 1500))
# save
ggsave("output/densities_revision.pdf", plot = dens1, device = "pdf",
